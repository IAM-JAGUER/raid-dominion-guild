-- ============================================================
-- RaidDominion Portal — El reclamo ya NO auto-asigna el personaje principal
--
-- BUG: `raiddominion_claim_from_sv` (canónica 20260905) sobreescribía
-- `raiddominion_profiles.character_name` con el player del SV:
--
--     UPDATE ... SET role='guild_master', is_guild_master=TRUE,
--         character_name = COALESCE(v_raw -> 'player' ->> 'name', character_name)
--
-- Consecuencia: subir un SV con registry.guild.isGM (reclamo auto-GM) asignaba
-- silenciosamente ese personaje como "Personaje principal" del perfil, aunque
-- el usuario lo hubiera dejado en "Ninguno" o hubiera elegido OTRO personaje
-- en la pestaña de configuración del dashboard.
--
-- DECISIÓN (2026-09-01): `character_name` es SOLO la elección explícita del
-- usuario en el dashboard (select "Personaje principal", alimentado por
-- personajes públicos). Ningún flujo de upload/reclamo lo toca. El reclamo
-- solo promueve rol y marca maestría (is_guild_master), conservando lo que el
-- usuario haya configurado.
--
-- Reglas: solo tablas/funciones raiddominion_, IF EXISTS, SECURITY DEFINER +
-- SET search_path = '', sin tocar handle_new_user() ni otras apps.
-- ============================================================
...
BEGIN
    ...
    -- NUNCA se toca character_name: el "Personaje principal" es elección
    -- explícita del usuario en el dashboard, no un derivado del upload.
