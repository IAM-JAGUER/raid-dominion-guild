-- ============================================================
-- RaidDominion Portal — Reclamo desde SV para staff (admin/moderator)
--
-- Decisión (2026-09-02): una cuenta admin/moderator que sube un SV donde el
-- addon acredita isGM debe poder reclamar su hermandad igual que una cuenta
-- member validada (paridad con el flujo visitante→member→guild_master).
--
-- Cambios sobre la versión 20260830:
--   * Gate de rol: NOT IN ('member','guild_master') → admite también
--     'moderator' y 'admin' (solo queda excluido 'visitante'/NULL).
--   * La evidencia anti-falso-positivo se MANTIENE para staff: si el rol no es
--     guild_master, exige >= 3 personajes VALIDADOS (igual que member).
--   * Sin degradar staff: al reclamar, admin/moderator conservan su rol; solo
--     las cuentas member pasan a guild_master. is_guild_master y user_apps.role
--     sí se actualizan para que el dashboard/hermandad funcionen.
--
-- Se conserva TODO lo vigente del claim: multi-GM, guard anti-falso-positivo
-- (descartar candidata ya registrada por otro maestro) y ranks en el snapshot.
-- ============================================================

-- ─── Reclamo de hermandad desde el SV (staff incluido) ──────────────────
