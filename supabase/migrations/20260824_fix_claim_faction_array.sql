-- ============================================================
-- RaidDominion Portal — Fix claim: characters es JSONB ARRAY
--
-- Error verificado 2026-08-24 al reclamar hermandad desde el SV:
--   cannot call jsonb_each on a non-object
--
-- El parser guarda `raw.characters` como ARRAY de personajes
-- [{name, realm, faction, ...}], pero raiddominion_claim_from_sv
-- hacía jsonb_each() (exige objeto tipo mapa "Nombre-Reino").
-- Se soportan AMBAS formas: array (parser actual) y objeto
-- (legacy/mapas), para que la facción del personaje activo se
-- resuelva sin reventar el reclamo.
-- ============================================================
...
BEGIN
    ...
    -- raw.characters es ARRAY en el parser actual; objeto (mapa) en legacy.
    IF v_player ? 'name' AND v_raw ? 'characters' THEN
        IF jsonb_typeof(v_raw -> 'characters') = 'array' THEN
            SELECT value ->> 'faction' INTO v_faction
            FROM jsonb_array_elements(v_raw -> 'characters') AS value
            WHERE lower(COALESCE(value ->> 'name', '')) = lower(v_player ->> 'name')
              AND (v_realm IS NULL OR lower(COALESCE(value ->> 'realm', '')) = lower(v_realm))
            LIMIT 1;
        ELSIF jsonb_typeof(v_raw -> 'characters') = 'object' THEN
            SELECT value ->> 'faction' INTO v_faction
