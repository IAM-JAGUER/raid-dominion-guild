-- ============================================================
-- RaidDominion Portal — Asignación de reglas por banda (catálogo SV)
--
-- Decisión (2026-09-04):
--   * El SV aporta el CATÁLOGO de reglas (lista global del addon). El usuario
--     decide qué reglas aplican a CADA banda desde su ficha en /dashboard#bandas
--     (varias bandas pueden compartir la misma regla; modelo embedded rules[]).
--   * NUEVO RPC raiddominion_set_band_rules: persiste el subset asignado por el
--     dueño a una banda (valida propiedad).
--   * upsert_bands ya NO pisa rules en re-upload: se PRESERVAN las reglas
--     asignadas manualmente (mismo criterio que guild_id/is_public). En el
--     INSERT inicial la banda hereda el catálogo del SV como punto de partida.
-- ============================================================

-- ─── 1) RPC raiddominion_set_band_rules ─────────────────────────────────
-- El dueño de la banda fija su lista de reglas (JSONB array de
-- {title, content, icon}). Valida propiedad; p_rules NULL → [].
DROP FUNCTION IF EXISTS public.raiddominion_set_band_rules(UUID, JSONB);
CREATE OR REPLACE FUNCTION public.raiddominion_set_band_rules(
    p_band_id UUID,
    p_rules JSONB
)
RETURNS BOOLEAN
...
GRANT EXECUTE ON FUNCTION public.raiddominion_set_band_rules(UUID, JSONB) TO authenticated;

-- ─── 2) upsert_bands: preservar rules en re-upload ──────────────────────
-- La rama UPDATE ya NO escribe rules: la selección manual del dueño sobrevive
-- a cada re-upload. El catálogo nuevo del SV sigue disponible vía getMyRules
-- (el usuario decide si lo asigna). El INSERT inicial conserva el catálogo
-- global del SV como default.
