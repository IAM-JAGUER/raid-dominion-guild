-- ============================================================
-- RaidDominion Portal — Guard anti-falso-positivo en el reclamo
--
-- Regla de producto (2026-08-25):
--   1. Al reclamar desde el SV, si OTRO maestro ya tiene registrada
--      una hermandad con el MISMO nombre, el candidato se DESCARTA:
--      no se crea un duplicado (evita falsos positivos). Solo el
--      dueño existente puede re-verificarla en su re-upload.
--   2. Se elimina por completo el reclamo MANUAL
--      (raiddominion_claim_guild): ningún formulario puede inventar
--      un nombre de hermandad que no exista en un SV. La única vía
--      para convertirse en guild_master es que el SV acredite
--      isGM=true y se cumpla todo el flujo de requisitos.
-- ============================================================
...
-- ─── Reclamo manual: ELIMINADO. Nadie puede reclamar a mano un nombre
-- de hermandad que no exista en un SV (la única vía es el SV con isGM).
DROP FUNCTION IF EXISTS public.raiddominion_claim_guild(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT);